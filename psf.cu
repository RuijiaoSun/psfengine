...
void gpuPSFSubstrate(cuda::std::complex<float>* field, unsigned int field_res, float extent, float w, int axis, 
    ...
    unsigned int array_size = sizeof(tira::planewave<float>) * N;

    tira::planewave<float>* gpu_I;
    HANDLE_ERROR(cudaMalloc(&gpu_I, array_size));
    HANDLE_ERROR(cudaMemcpy(gpu_I, I, array_size, cudaMemcpyHostToDevice));

    tira::planewave<float>* gpu_R;
    HANDLE_ERROR(cudaMalloc(&gpu_R, array_size));
    HANDLE_ERROR(cudaMemcpy(gpu_R, R, array_size, cudaMemcpyHostToDevice));

    tira::planewave<float>* gpu_T;
    HANDLE_ERROR(cudaMalloc(&gpu_T, array_size));
    HANDLE_ERROR(cudaMemcpy(gpu_T, T, array_size, cudaMemcpyHostToDevice));
